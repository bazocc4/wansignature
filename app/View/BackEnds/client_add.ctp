	...
	<fieldset>
		<script>
			$(document).ready(function(){
				...
				// CUSTOMIZED SCRIPT !!
                $('input[type=radio].kategori').change(function(){
                    if($(this).is(':checked'))
                    {
                        if($(this).val() == 'Retailer')
                        {
                            $('input#wholesaler').closest('.control-group').show();
                        }
                        else
                        {
                $('input#wholesaler').closest('.control-group').hide();
                $('input#wholesaler').val('').nextAll('input[type=hidden].wholesaler').val('');
                        }
                    }
                }).trigger('change');
			});
