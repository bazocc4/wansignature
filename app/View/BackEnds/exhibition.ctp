...
-->
		...
		<?php
			if(empty($popup))
			{
                ...
                echo $this->Html->link('<i class="icon-retweet icon-white"></i>', array('action' => 'surat-jalan', '?' => array('storage'=>'exhibition', 'content'=>$value['Entry']['slug'] ) ) , array('escape'=>false, 'class'=>'btn btn-primary','data-toggle'=>'tooltip', 'title'=>'CLICK TO VIEW ALL SHOWPIECES AT THIS EXHIBITION', 'target'=>'_blank') ); // view semua barang pameran di exhibition terpilih ...
                echo '&nbsp;&nbsp;';
