		...
		<?php
			...
			foreach ($myAutomatic as $key => $value)
			{
				if(substr($value['key'], 0 , 5) == 'form-')
				{
                    ...
                    // multibrowse special case, from Account Model ...
                    if($value['key'] == 'form-warehouse_employee')
                    {
                        $value['controller'] = 'accounts';
                        $value['action'] = 'none';
                        $value['request_query'] = array('role' => str_replace('_', '+', substr($value['key'], 5)) );
                    }
