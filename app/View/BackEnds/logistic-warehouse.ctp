	...
	<tr class="orderlist" alt="<?php echo $value['Entry']['id']; ?>">
		<td class="main-title">
			<?php
                ...
                // renew Entry title ...
                $getwh = $this->Get->meta_details($value['Entry']['title'] , 'warehouse');
                $value['Entry']['title'] = $getwh['Entry']['title'];
                if(empty($value['Entry']['description']))
                {
                    $value['Entry']['description'] = $getwh['EntryMeta']['alamat'];
                }
			?>
...
*/
				?>
            &nbsp;<a data-toggle="tooltip" title="CLICK TO DELETE RECORD" href="javascript:void(0)" onclick="show_confirm('Are you sure want to delete this warehouse stock ?','entries/delete/<?php echo $value['Entry']['id']; ?>')" class="btn btn-danger"><i class="icon-trash icon-white"></i></a>
				<?php
				echo "</td>";
			}				
		?>
